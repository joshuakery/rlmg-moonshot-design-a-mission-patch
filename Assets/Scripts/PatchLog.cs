    ...
    public class PatchLog : MonoBehaviour
    {
        ...
        public void UpdatePatches()
        {
            for (int i = 0; i < patches.Length; i++)
            {
                ...
                if (gameState.savedScanManager.scans != null && gameState.savedScanManager.scans.Length > i)
                {
                    if (gameState.savedScanManager.scans[i] != null) //if we have a scan, show it
                    {
                        ...
                        if (patch.drawingGenericWindow != null)
                            patch.drawingGenericWindow.Open();

                        if (patch.defaultGenericWindow != null)
