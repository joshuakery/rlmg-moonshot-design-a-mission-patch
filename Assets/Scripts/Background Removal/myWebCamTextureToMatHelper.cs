...
namespace OpenCVForUnity.UnityUtils.Helper
{
    ...
    /// v 1.1.3
    /// </summary>
    public class myWebCamTextureToMatHelper : MonoBehaviour
    {
        /// <summary>
        ...
        protected virtual IEnumerator _Initialize()
        {
            ...
            // Debug.Log("Requested Device Name: " + requestedDeviceName);
            ...
            if (webCamTexture == null)
            {
                ...
                for (int cameraIndex = 0; cameraIndex < devices.Length; cameraIndex++)
                {
                    // Debug.Log("Webcam Texture is null, so choosing a device based on frontfacing request");
            ...
            if (webCamTexture == null)
            {
                if (devices.Length > 0)
                {
                    // Debug.Log("Setting webCamDevice to first device");
...
/*                initFrameCount++;
                ...
                if (webCamTexture.didUpdateThisFrame)
                {
                    ...
                    RLMGLogger.Instance.Log(deviceInfo, MESSAGETYPE.INFO);
