...
public class ResultsDisplay : MonoBehaviour
{
    ...
    public void LoadTeamResults()
    {
        if (useServer == true)
        {
            if (Client.instance.team == null)
            {
                ...
                RLMGLogger.Instance.Log("Created new team data because none existed. This should only happen when testing the results scene in isolation.", MESSAGETYPE.ERROR);
    ...
    public void LoadTeamResults(MoonshotTeamData singleTeamData)
    {
        ...
        RLMGLogger.Instance.Log("Loading Team Results Data   teamName = " + singleTeamData.teamName, MESSAGETYPE.INFO);
    ...
    public IEnumerator DownloadThenLoadImages(MoonshotTeamData singleTeamData)
    {
        ...
        RLMGLogger.Instance.Log("ResultsDisplay.DownloadThenLoadImages() singleTeamData = " + singleTeamData, MESSAGETYPE.INFO);

        if (singleTeamData == null)
        {
            //Debug.LogError("singleTeamData == null");
            RLMGLogger.Instance.Log("singleTeamData == null", MESSAGETYPE.ERROR);
        ...
        if (doTestUploadFirst)
        {
            ...
            RLMGLogger.Instance.Log("test artwork file(s) about to be uploaded.   Time.time = " + Time.time, MESSAGETYPE.INFO);
    ...
    public IEnumerator LoadImagesViaFilenames(string directoryPath, string[] filenames)
    {
        ...
        for (int i = 0; i < filenames.Length; i++)
        {
            if (!string.IsNullOrEmpty(filenames[i]))
            {
                ...
                RLMGLogger.Instance.Log("trying to load image from file path: " + imgFilePath + "   directory exists? " + Directory.Exists(Path.GetDirectoryName(Path.Combine(directoryPath, filenames[i]))), MESSAGETYPE.ERROR);


                yield return StartCoroutine(ContentLoader.LoadSpriteFromFilepath(imgFilePath, result => artworkSprites[i] = result));

                //Debug.Log("artworkSprites #" + i + " = " + artworkSprites[i]);
                RLMGLogger.Instance.Log("artworkSprites #" + i + " = " + artworkSprites[i], MESSAGETYPE.INFO);
            }
        }
