...
public class ConclusionTextManager : MonoBehaviour
{
    ...
    void OnEnable()
    {
        if (Client.instance == null || Client.instance.team == null || Client.instance.team.MoonshotTeamData == null)
        {
            ...
            RLMGLogger.Instance.Log("ConclusionTextManager couldn't find team data through Client.instance", MESSAGETYPE.INFO);
    ...
    void UpdateText(ConclusionTextMatrix contentMatrix)
    {
        ...
        RLMGLogger.Instance.Log("ConclusionTextManager    roverSuccess=" + roverSuccess + "   mapSuccess=" + mapSuccess + "   artSuccess=" + artSuccess + "   charterSuccess=" + charterSuccess + "   treasureSuccess=" + treasureSuccess, MESSAGETYPE.INFO);
        ...
        for (int i = 0; i < contentMatrix.options.Length; i++)
        {
                ...
                treasureSuccess == System.Convert.ToBoolean(contentMatrix.options[i].treasureSuccess))
            {
                ...
                RLMGLogger.Instance.Log("dynamic conclusion text: " + contentMatrix.options[i].sentence1 + "\n\n" + contentMatrix.options[i].sentence2);
